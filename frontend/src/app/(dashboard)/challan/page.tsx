...
const HINDI_DICTIONARY: { [key: string]: string } = {
  ...
  "PIPE REENA MEDIUM 25 MM": "रीना मीडियम पाइप 25 मिमी",
                ...
                {challanData.items.filter((item: any) => item.qty > 0).length > 0 ? (
                  challanData.items.filter((item: any) => item.qty > 0).map((item: any, idx: number) => (
                    <tr key={item.id} className="uppercase text-slate-900">
                      ...
                      <td className="py-1 px-2 border-r border-slate-300 text-slate-955 font-extrabold">{translateBilingual(item.material)}</td>
                      <td className="py-1 px-2 border-r border-slate-300 text-right font-mono text-slate-955 w-24">{item.qty}</td>
                      <td className="py-1 px-2 text-center font-bold text-slate-500 w-20">{item.unit}</td>
                    </tr>
                  ))
                ) : (
